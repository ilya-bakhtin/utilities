...
static void md5_process(md5_state_t *pms, const BYTE *data)
{
	CARD32	a = pms->abcd[0], b = pms->abcd[1],	c = pms->abcd[2], d = pms->abcd[3];
	CARD32 t;
...
void md5_append(md5_state_t* pms, const BYTE* data, size_t nbytes)
{
	const BYTE *p = data;
	size_t left = nbytes;
	size_t offset = (pms->count[0] >> 3) & 63;
	CARD32 nbits = (CARD32)(nbytes << 3);
...
void md5_finish(md5_state_t* pms, BYTE digest[16])
{
...
md5_hash calculate_md5(const void* buf, size_t length)
{
	...
	md5_init(&state);
	md5_append(&state, (const BYTE *)buf, length);
